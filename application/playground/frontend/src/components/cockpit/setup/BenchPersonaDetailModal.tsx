...
export function BenchPersonaDetailModal({
  ...
  return (
    <RailInsetModal
      ...
      title={persona?.name ?? (personaId ? `persona-${personaId}` : t("setup.persona.title", "Persona"))}
      subtitle={`${t("setup.persona.title", "Persona")} · ${activePool}`}
      onClose={onClose}
    >
      {detailQuery.isPending && (
        <p className="text-[14px] text-text-dim">{t("setup.persona.loadingRecord", "Loading persona record…")}</p>
      )}
      {detailQuery.isError && (
      ...
      {onUse && persona && (
        <div className="mt-4 flex justify-end border-t border-outline/30 pt-4">
          ...
          >
              {t("setup.persona.use", "Use persona")}
          </button>
        </div>
      )}
    </RailInsetModal>
