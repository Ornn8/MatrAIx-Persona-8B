...
export function PersonaCatalog({ selectedId, onSelect }: PersonaCatalogProps) {
  ...
  const totalLabel =
    personasQuery.isLoading && all.length === 0
      ? t("catalog.personaCatalog.loading", "Loading personas…")
      : t("catalog.personaCatalog.ready", "{count} personas ready", {
    ...
    <aside className="relative z-0 flex h-[260px] w-full flex-shrink-0 flex-col border-b border-outline bg-surface-lowest lg:h-full lg:w-[300px] lg:border-b-0 lg:border-r">
      ...
      <div className="flex-shrink-0 border-b border-outline p-md pb-sm">
        <div className="mb-xs">
          ...
          <h2 className="font-display text-[15px] font-bold text-text-main">
            {t("catalog.personaCatalog.pick", "Pick who to simulate")}
          </h2>
        </div>
        ...
        <div className="relative mb-sm w-full">
          ...
          <input
            ...
            placeholder={t(
              "catalog.personaCatalog.searchPlaceholder",
              "Search by role, age, or trait (e.g. \"manager\" or \"student\")",
            )}
        ...
        >
          <FilterChip
            label={t("catalog.personaCatalog.allSources", "All sources")}
            active={filter === "all"}
            onClick={() => setFilter("all")}
          />
          <FilterChip
            label={t("catalog.personaCatalog.curated", "Curated")}
            title={t("catalog.personaCatalog.curatedTitle", "Hand-picked personas we ship by default")}
