...
export function InspectorTabs({
  ...
  const tabs: Array<{ id: InspectorTab; label: string; icon: string }> = [
    { id: "evaluation", label: t("cockpit.inspector.evaluation", "Evaluation"), icon: "verified" },
    { id: "instruction", label: t("cockpit.inspector.instruction", "Instruction"), icon: "description" },
  ];
  if (context) tabs.push({ id: "context", label: t("cockpit.inspector.context", "Context"), icon: "menu_book" });
  if (questionnaire) tabs.push({ id: "questionnaire", label: t("cockpit.inspector.questionnaire", "Questionnaire"), icon: "list_alt" });
  if (outputSchema) tabs.push({ id: "output-schema", label: t("cockpit.inspector.outputSchema", "Output schema"), icon: "schema" });
  if (selfReport) tabs.push({ id: "self-report", label: t("cockpit.inspector.selfReport", "Self-report"), icon: "rate_review" });
  ...
  return (
    <aside className="z-0 flex h-[340px] w-full flex-shrink-0 flex-col border-t border-outline bg-surface-lowest lg:h-full lg:w-[360px] lg:border-l lg:border-t-0">
      <div className="flex shrink-0 items-center justify-between border-b border-outline bg-surface px-4 py-3">
        <span className="hud text-[12px] text-primary">{t("cockpit.inspector.title", "Inspector")}</span>
        <span className="hud text-[11px] text-text-dim">{activeLabel}</span>
      </div>

      <div
        role="tablist"
        aria-label={t("cockpit.inspector.title", "Inspector")}
