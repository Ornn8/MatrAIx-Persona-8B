  ...
  return (
    <div className="fixed inset-0 z-50">
      ...
      <div
        role="dialog"
        aria-modal="true"
        aria-label={t("catalog.personaDrawer.dialogLabel", "Full profile for {title}", { title })}
      ...
      >
          ...
          <div className="flex items-start justify-between gap-3">
            <div className="flex min-w-0 items-start gap-3.5">
              ...
              <div className="min-w-0">
                <div className="hud mb-1.5 text-[11px] text-text-dim">
                  {t("catalog.personaDrawer.persona", "Persona")}
                </div>
            ...
            <button
              ...
              type="button"
              onClick={onClose}
              aria-label={t("catalog.personaDrawer.close", "Close persona detail")}
        ...
        <div className="custom-scrollbar flex-1 space-y-5 overflow-y-auto p-5">
          {loading ? (
            <div
              className="space-y-2"
              aria-label={t("catalog.personaDrawer.loading", "Loading full persona")}
              aria-busy
            >
              {[5, 7, 6, 4, 7, 5].map((w, i) => (
                <div
            ...
            <>
              ...
              <div className="panel rise-in rounded-md border border-outline bg-surface p-4">
                <h3 className="hud mb-3 text-[12px] text-text-dim">
                  {t("catalog.personaDrawer.demographics", "Demographics")}
                </h3>
                {demographics.length > 0 ? (
                    ...
                    {demographics.map((d) => (
                      <div key={d.key} className="flex items-start justify-between gap-3">
                        <span className="hud flex-none text-[11px] text-text-dim">
                          {t(`catalog.persona.dimension.${d.key}`, humanizeToken(d.key))}
                        </span>
                  ...
                  <p className="text-[14px] italic leading-snug text-text-variant">
                    {t(
                      "catalog.personaDrawer.noDemographics",
              ...
              {sections.length > 0 && (
                ...
                >
                  <h3 className="hud mb-3 text-[12px] text-text-dim">
                    {t("catalog.personaDrawer.goalContext", "Goal context")}
                  </h3>
              ...
              {rawText ? (
                ...
                >
                  <div className="mb-3 flex items-center justify-between gap-3">
                    <h3 className="hud text-[12px] text-text-dim">
                      {t("catalog.personaDrawer.rawRecord", "Raw record")}
                    </h3>
                    <button
                      type="button"
                      onClick={handleCopy}
                      aria-label={
                        copied
                          ? t("catalog.personaDrawer.copiedToClipboard", "Copied to clipboard")
                          : t("catalog.personaDrawer.copyRawRecord", "Copy raw record")
                      }
                    ...
                    >
                      ...
                      <span className="hud text-[11px]">
                        {copied
                          ? t("catalog.personaDrawer.copied", "Copied")
                          : t("catalog.personaDrawer.copy", "Copy")}
                      </span>
                    </button>
                  </div>
                ...
                <p className="text-[14px] italic leading-relaxed text-text-variant">
                  {t(
                    "catalog.personaDrawer.noExtraProfile",
        ...
        {onUse && (
          <div className="flex-shrink-0 border-t border-outline p-4">
            ...
            >
              ...
              {t("catalog.personaDrawer.use", "Use this persona")}
            </button>
          </div>
        )}
      </div>
